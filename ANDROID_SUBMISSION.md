...
- ✅ **한국 개발자 추가 정보(전자상거래법) 해결** — 유료 인앱 상품 생성 시 Play Console > 개발자 계정 > 계정 세부정보 > "한국 개발자의 경우 추가 정보 필요" 입력 필수가 됨:
    - **사업자 등록 번호** = `1502302302` (ASC 대한민국 세금 양식에서 확인. 사용자 보유) → 입력·저장 완료
    - 전자상거래 라이선스 번호(통신판매업)·대행사 = **선택사항**(사용자가 통신판매업 신고 미보유, 비워둬도 저장됨)
    - ⚠️ 함정: 이 정보 없으면 `supply` AAB 업로드가 "To comply with Korean law..." 에러로 차단됨. 사업자등록번호만 입력하면 통과.
- ✅ **vc9(결제 코드) AAB → alpha 트랙 업로드 + 심사 전송** — `supply --aab`. vc9가 새 알파 릴리스로 생성되어 사전검사→심사 진행 중. 승인 시 테스터에게 게시(vc8은 그때까지 라이브 유지).
- ⬜ **(자동) 심사 승인 후** 테스터 vc9 설치 → 클로즈드 테스트에서 **샌드박스 결제 검증**(구매 → 서버 `verifyGooglePurchase` → 크레딧 지급)
- ⚠️ **(사용자, 정산 시)** Play 결제 프로필에 정산 은행 계좌 미등록 — 실제 수익 정산 전 추가 필요(상품 판매/테스트에는 불필요)
