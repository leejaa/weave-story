...
const JudgeSchema = z.object({
  ...
  genreFit: z.number().int().min(0).max(2), // 장르 적합: 목표 장르 레지스터(톤·감성)의 결에 맞나(0 엉뚱·이탈 ~ 2 정확)
  emotionalDelivery: z.number().int().min(0).max(2), // 정서 전달: emotionalCore(감성·메시지)가 실제로 와닿나(0 밋밋 ~ 2 와닿음)
  ...
  try {
    ...
    const prompt = [
      ...
      [
        ...
        '- genreFit 0-2: does the chapter land squarely in the TARGET genre register / feel stated above (the EMOTIONAL CORE + tone)? (0=wrong feel, off-register, 2=exactly the register\'s feel).',
        '- emotionalDelivery 0-2: does the chapter actually evoke the EMOTIONAL CORE (the intended feeling + message)? (0=flat / does not land, 2=the feeling lands).',
        ...
        '- genreDrift: did it drift from the GIVEN genre\'s FEEL — especially (for non-mystery genres) sliding toward mystery/thriller: escalating dread/peril, making "uncover the hidden secret" the engine, or a head-scratching puzzle the reader must solve? (true = problem)',
    ...
    // 점수식: 장르 적합·정서 전달·가독성·개연성을 무겁게, 장르 느낌 이탈·복잡도·인과 비약을 강하게 감점.
    const base = (output.coherence / 5) * 15 + (output.arcAdvance / 2) * 5
      + (output.protagonistFocus / 2) * 5 + (output.readability / 2) * 15
      + (output.simplicity / 2) * 10 + (output.motivation / 2) * 15
      + (output.genreFit / 2) * 15 + (output.emotionalDelivery / 2) * 15
      + (output.choiceExecuted ? 5 : 0);
    ...
    await params.db.insert(chapterJudgements).values({
      ...
      scores: {
        ...
        genreFit: output.genreFit,
        emotionalDelivery: output.emotionalDelivery,
    ...
    console.log(`[judge] thread=${params.threadId} chapter=${params.chapterNumber} overall=${overall} coh=${output.coherence} genreFit=${output.genreFit} emo=${output.emotionalDelivery} read=${output.readability} mot=${output.motivation} flags=${flagList}`);
  } catch (err) {
