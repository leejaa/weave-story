...
const JA_CHOICE_RULES = [
  ...
  '- [根拠] 選択肢は本文・アウトラインで既に確立した事実のみを前提にします。本文に出ていない新事実(事故・死因・原因・場所・人物・設定など)を断定したり捏造しないでください。(例:「昏睡」しか出ていないのに「事故現場へ向かう」のように原因を捏造禁止)',
  '- [収束] 2つの選択肢はいずれも、既に開かれた状況とアウトラインが矛盾なく吸収できる経路でなければなりません。物語を設計の外へ弾き出す分岐を作らないでください。',
