...
const KO_PROSE_GUARDRAIL = [
  '[문체 가드레일 — 쉽고 빠르게 읽히는 글 (최우선)]',
  '- 쉽고 일상적인 단어로 쓰세요. 어려운 한자어·관념어·문학적 수사를 피하고, 누구나 한 번에 이해되는 표현만 씁니다.',
  '- 문장은 짧고 명료하게. 한 문장에 한 가지만 담으세요. 길고 만연한 문장은 금지합니다.',
  '- 역설·대구 구문("X가 아니라 Y였다", "...할수록 ...했다")은 한 화에 최대 1번. 사실상 쓰지 마세요.',
  '- 추상적 내면 독백·관념적 서술을 늘어놓지 마세요. 감정은 행동·대사·구체적 장면으로 보여줍니다(telling보다 showing).',
  '- 같은 비유·모티프(빛·그림자·심장·온도 등)를 반복하지 마세요.',
  '- 대화를 적극적으로 사용해 장면을 빠르고 가볍게 굴리세요.',
  '- 설명조로 정보를 쏟아붓지 말고, 꼭 필요한 것만 자연스럽게 흘리세요.',
].join('\n');
...
const KO_CHOICE_RULES = [
  ...
  '- [근거] 선택지는 본문·아웃라인에 이미 확립된 사실만 전제로 합니다. 본문에 나온 적 없는 새 사실(사고·사인·원인·장소·인물·설정 등)을 단정하거나 지어내지 마세요. (예: "혼수"만 나왔는데 "사고 현장으로 간다"처럼 원인을 날조 금지)',
  '- [수렴] 두 선택지 모두, 이미 열린 상황과 아웃라인이 모순 없이 흡수할 수 있는 경로여야 합니다. 이야기를 설계 밖으로 튕겨내는 분기를 만들지 마세요.',
