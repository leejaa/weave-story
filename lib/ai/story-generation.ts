...
async function generateObjectWithRetry<T extends z.ZodTypeAny>(
  ...
  try {
    ...
    const isSchemaFailure =
      err instanceof Error && err.message.includes('response did not match schema');
...
const FirstChapterSchema = z.object({
  ...
  situation: z
    .string()
    .max(80)
    .describe('【선택지 화면에 표시되는 짧은 상황 요약】 content에서 묘사한 결정 순간을 1-2문장으로만 요약. 60자 이내. 새로운 내용 금지.'),
  question: z
    .string()
    .max(30)
    .describe('독자에게 던지는 선택 질문 (예: "서연은 어떻게 할 것인가?", 20자 이내)'),
  choices: z
    .array(z.string().max(30))
    .length(2)
    .describe('독자 선택지 정확히 2개 (각 25자 이내, 한국어, 구체적인 행동 묘사)'),
});
...
function validateChapterResult(
  ...
  if (result.content.length < 1000) {
    issues.push(`content too short: ${result.content.length} chars (expected 2000+)`);
  }
  ...
  if (issues.length > 0) {
    ...
    console.warn(`${tag} situation="${result.situation}" question="${result.question}" choices=${JSON.stringify(result.choices)}`);
  } else {
    console.log(`${tag} validation OK content=${result.content.length} choices=${result.choices.length}`);
  }
}
...
- content와 situation에 같은 내용을 중복으로 쓰지 마세요.`,
  ...
  console.log(`[gen] first chapter done title="${object.title}" genre=${object.genre} content=${object.content.length} choices=${object.choices.length}`);
