...
const OLD_65_SET = '3014b0ad-f362-43c0-896e-6af8c12fc5a4'; // en-US old 6.5" set (old design)
...
const v = (await asc('GET', `/v1/apps/${APP_ID}/appStoreVersions?limit=1`)).data[0];
...
for (const loc of ['en-US', 'ja', 'ko']) {
  ...
  // remove any existing 6.7" set for this locale (clean re-run)
  ...
  for (const s of sets.data) {
    if (s.attributes.screenshotDisplayType === DISPLAY) {
      ...
      console.log(`  [${loc}] removed existing 6.7" set`);
    }
  }
  ...
  for (const ord of ORDER) {
    const file = `ios-${FILE_LOCALE[loc]}-${ord}.png`;
  ...
  console.log(`  [${loc}] 6.7" set complete (${shotIds.length})`);
}

// delete the old en-US 6.5" set (old design)
try {
  await asc('DELETE', `/v1/appScreenshotSets/${OLD_65_SET}`);
  console.log('deleted old en-US 6.5" set ✓');
} catch (e) { console.log('old 6.5" delete note:', String(e).slice(0, 140)); }
